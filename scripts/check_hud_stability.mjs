...
// monitor: blank-frame + backing-store changes across resizes
const stats = await page.evaluate(() => new Promise((resolve) => {
  ...
  const sample = () => {
    ...
    else resolve({ blank, total, storeChanges, minCoverage: "sampled" });
  };
  requestAnimationFrame(sample);
}));

// height-only (mobile URL bar) then width resizes
